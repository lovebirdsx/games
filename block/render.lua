...
function render.init()
	...
	_img_ice = love.graphics.newImage('image/ice.png')
...
function render.draw_hex1(c, x, y, scale, alpha)
...
function render.draw_hex(c, x, y, scale, alpha)
	...
	local offset_x = w * scale / 2
	local offset_y = h * scale / 2
	love.graphics.draw(_img_ice, x - offset_x, y - offset_y, 0, scale, scale)
end
